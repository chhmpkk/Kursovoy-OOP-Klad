...
namespace KLAD
{
    public partial class MainWindow : Window
    {
        ...
        private void StartNewGame()
        {
            ...
            // Очищаем существующие сокровища и призы с карты (если были из BMP), чтобы спавнить точно по настройкам
            for (int x = 0; x < _gameState.Level.Width; x++)
            {
                for (int y = 0; y < _gameState.Level.Height; y++)
                {
                    if (_gameState.Level.Grid[x, y].Type == ElementType.Treasure || _gameState.Level.Grid[x, y].Type == ElementType.Prize)
                    {
                        _gameState.Level.Grid[x, y] = new EmptySpace();
                    }
                }
        ...
        private void CheckWinCondition()
        {
            ...
            else if (_gameState.Player1.Score + _gameState.Player2.Score == _gameState.TotalTreasures)
            {
                ...
                {
                    ShowGameOver("Ничья! Победила дружба.", false);
                }
            }
        }
